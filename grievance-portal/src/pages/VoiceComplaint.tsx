      ...
      <main className="container mx-auto px-4 py-8">
        <section className="rounded-2xl border bg-white p-6 shadow-sm md:p-8">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            {confirmed ? <div className="w-full rounded-xl border bg-slate-50 p-5 text-left"><h3 className="mb-3 text-lg font-semibold">Submission</h3><div className="mb-3 rounded-lg border bg-white p-3 text-sm"><div><span className="font-semibold">Category:</span> {category || "Not set"}</div><div><span className="font-semibold">Language:</span> {langs.find((l) => l.code === language)?.label}</div><div><span className="font-semibold">Description:</span> {description || "Not set"}</div><div><span className="font-semibold">Location:</span> {address || (coords ? `${coords.lat}, ${coords.lng}` : "Not set")}</div><div><span className="font-semibold">Photos:</span> {images.length}</div></div><Button className="min-h-[60px] bg-gradient-to-r from-emerald-600 to-green-500 hover:from-emerald-700 hover:to-green-600" onClick={submit} disabled={submitting}>{submitting ? "Submitting..." : "Submit Complaint"}</Button>{ok ? <div className="mt-4 rounded-lg border border-emerald-300 bg-emerald-50 p-4"><div className="flex items-center gap-2 text-emerald-700"><CheckCircle2 className="h-6 w-6 animate-pulse" /><span className="font-semibold">Success</span></div><p className="mt-2 text-sm text-emerald-800">Complaint ID: <span className="font-semibold">{complaintId}</span></p><div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" onClick={resetAll}>File Another</Button><Button onClick={() => nav("/my-complaints")}>View Status</Button></div></div> : null}</div> : null}

            <div className="w-full rounded-xl border bg-white text-left"><button type="button" onClick={() => setHelpOpen((p) => !p)} className="flex min-h-[60px] w-full items-center justify-between px-4 py-3 font-semibold">Need Help?<ChevronDown className={`h-5 w-5 transition ${helpOpen ? "rotate-180" : ""}`} /></button>{helpOpen ? <div className="border-t px-4 py-4"><div className="text-sm font-semibold">Tips</div><ul className="mt-2 space-y-1 text-sm">{help[language].tips.map((t) => <li key={t}>- {t}</li>)}</ul><div className="mt-3 text-sm font-semibold">Example phrases</div><ul className="mt-2 space-y-1 text-sm">{help[language].ex.map((x) => <li key={x}>- {x}</li>)}</ul><div className="mt-3 text-sm font-semibold">Helpline: 1800-XXX-XXXX</div></div> : null}</div>
          </div>
        </section>
      </main>
    </div>
