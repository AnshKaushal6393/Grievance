...
const AdminUserManagement = () => {
  ...
  const roleLabel = (r: UserRole) =>
    r === "user"
      ? t("adminUsers.citizen", "Citizen")
      : r === "officer"
        ? t("adminUsers.officer", "Officer")
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main id="main-content" className="container mx-auto px-4 py-8">
        <ComplianceInfoBlock
          className="mb-6"
          source={t("compliance.adminUsers.source", "User administration register with live account controls")}
          lastSync={new Date().toLocaleString("en-IN")}
          auditReference={t("compliance.adminUsers.auditRef", "ADM-USERS")}
          retentionNotice={t("compliance.adminUsers.retention", "User account changes, security actions, and role assignments are retained for audit review.")}
        />
        ...
        <Card><CardContent className="p-0">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b p-4">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="ghost" size="icon" className={focusRingClass} onClick={() => loadUsers()} aria-label={t("adminUsers.refreshList", "Refresh user list")}><RefreshCcw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /></Button>
            </div>
          </div>
            ...
            {loading ? <TableRow><TableCell colSpan={12} className="py-8 text-center">{t("common.loading", "Loading...")}</TableCell></TableRow> :
              users.length === 0 ? <TableRow><TableCell colSpan={12} className="py-8 text-center">{t("adminUsers.noUsers", "No users found")}</TableCell></TableRow> :
                users.map((u) => <TableRow key={u._id}>
                  ...
                  <TableCell><div className="flex gap-1"><Button variant="ghost" size="icon" className={focusRingClass} onClick={() => openDetails(u._id)} aria-label={t("adminUsers.aria.viewUserDetails", "View user details")}><Eye className="h-4 w-4" /></Button>{u.status === "banned" ? <Button variant="ghost" size="icon" className={focusRingClass} onClick={() => setConfirm({ type: "unban", userId: u._id, userName: u.name })} aria-label={t("adminUsers.aria.unbanUser", "Unban user")}>{t("adminUsers.unban", "Unban")}</Button> : <Button variant="ghost" size="icon" className={focusRingClass} onClick={() => setConfirm({ type: "ban", userId: u._id, userName: u.name })} aria-label={t("adminUsers.aria.banUser", "Ban user")}><Ban className="h-4 w-4 text-red-600" /></Button>}<Button variant="ghost" size="icon" className={focusRingClass} onClick={() => setConfirm({ type: "delete", userId: u._id, userName: u.name })} aria-label={t("adminUsers.aria.deleteUser", "Delete user")}><Trash2 className="h-4 w-4 text-red-700" /></Button></div></TableCell>
                </TableRow>)}
          </TableBody></Table></div>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{t("adminUsers.userDetails", "User Details")}</DialogTitle><DialogDescription>{t("adminUsers.userDetailsSub", "Profile, activity and admin controls")}</DialogDescription></DialogHeader>
        {detailsLoading || !detail ? <div className="py-8 text-center text-muted-foreground">{t("common.loading", "Loading...")}</div> : <div className="space-y-4">
          <div className="rounded-lg border p-4"><p className="text-lg font-semibold">{detail.name}</p><p className="text-sm text-muted-foreground">{detail.email} • {detail.phone}</p><div className="mt-2 flex gap-2"><Badge>{roleLabel(detail.role)}</Badge><Badge variant="outline">{t("adminUsers.joinedOn", "Joined {date}").replace("{date}", new Date(detail.joinedDate).toLocaleDateString())}</Badge></div><p className="mt-2 text-sm">{t("adminUsers.verificationSummary", "Verification: Email {email} | Phone {phone} | Aadhaar {aadhaar}").replace("{email}", detail.verification.email ? "✓" : "✗").replace("{phone}", detail.verification.phone ? "✓" : "✗").replace("{aadhaar}", detail.verification.aadhaar ? "✓" : "✗")}</p></div>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3"><Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">{t("adminUsers.activity.totalComplaints", "Total complaints")}</p><p className="text-xl font-semibold">{detail.activity.totalComplaints}</p></CardContent></Card><Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">{t("adminUsers.activity.resolved", "Resolved")}</p><p className="text-xl font-semibold">{detail.activity.resolvedComplaints}</p></CardContent></Card><Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">{t("adminUsers.activity.avgRating", "Avg rating")}</p><p className="text-xl font-semibold">{detail.activity.avgRating}</p></CardContent></Card></div>
          <div className="rounded-lg border p-4"><p className="mb-2 font-medium">{t("adminUsers.activity.recent", "Recent activity")}</p><div className="space-y-2">{detail.activity.recentTimeline.length ? detail.activity.recentTimeline.map((t) => <div key={t.id} className="rounded-md bg-muted/40 p-2"><p className="text-sm font-medium">{t.label}</p><p className="text-xs text-muted-foreground">{t.description}</p></div>) : <p className="text-sm text-muted-foreground">{t("adminUsers.activity.none", "No activity")}</p>}</div></div>
          <div className="rounded-lg border p-4"><p className="mb-2 font-medium">{t("adminUsers.editOptions", "Edit options")}</p><div className="grid grid-cols-1 gap-3 md:grid-cols-2"><div><Label className="mb-2 block">{t("adminUsers.changeRole", "Change role")}</Label><Select value={editRole} onValueChange={(v: UserRole) => setEditRole(v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="user">{t("adminUsers.citizen", "Citizen")}</SelectItem><SelectItem value="officer">{t("adminUsers.officer", "Officer")}</SelectItem><SelectItem value="admin">{t("adminUsers.admin", "Admin")}</SelectItem></SelectContent></Select></div>{editRole === "officer" && <div><Label className="mb-2 block">{t("adminUsers.assignDepartment", "Assign department")}</Label><Select value={editDept} onValueChange={setEditDept}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">{t("adminUsers.selectDepartment", "Select department")}</SelectItem>{departments.map((d) => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent></Select></div>}</div><div className="mt-3 flex flex-wrap gap-2"><Button onClick={async () => { if (!detail) return; if (editRole === "officer" && editDept === "none") return toast.error(t("adminUsers.error.departmentRequiredOfficer", "Department required for officer")); setSaving(true); try { await adminService.updateUser(detail._id, { role: editRole, departmentId: editRole === "officer" ? editDept : undefined }); await openDetails(detail._id); await loadUsers(true); } catch (err: unknown) { toast.error(parseErr(err, t("adminUsers.error.updateUser", "Failed to update user"))); } finally { setSaving(false); } }}>{t("adminUsers.saveRoleDepartment", "Save role/department")}</Button>{detail.status === "banned" ? <Button variant="outline" onClick={() => setConfirm({ type: "unban", userId: detail._id, userName: detail.name })}>{t("adminUsers.unbanUser", "Unban user")}</Button> : <Button variant="destructive" onClick={() => setConfirm({ type: "ban", userId: detail._id, userName: detail.name })}>{t("adminUsers.banUser", "Ban user")}</Button>}<Button variant="outline" className="text-red-700 border-red-200 hover:bg-red-50" onClick={() => setConfirm({ type: "delete", userId: detail._id, userName: detail.name })}>{t("adminUsers.deleteUser", "Delete user")}</Button></div><div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-[1fr_auto]"><Input placeholder={t("adminUsers.customPasswordOptional", "Custom password (optional)")} value={resetPwd} onChange={(e) => setResetPwd(e.target.value)} /><Button variant="outline" onClick={() => setConfirm({ type: "reset", userId: detail._id, userName: detail.name })}>{t("adminUsers.resetPassword", "Reset Password")}</Button></div></div>
        </div>}
      </DialogContent></Dialog>

      <Dialog open={addOpen} onOpenChange={setAddOpen}><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle className="flex items-center gap-2"><UserPlus className="h-5 w-5" />{t("adminUsers.addUser", "Add User")}</DialogTitle></DialogHeader>
        <div className="space-y-3"><div className="grid grid-cols-1 gap-3 md:grid-cols-2"><div><Label className="mb-2 block">{t("adminUsers.name", "Name")}</Label><Input value={addForm.name} onChange={(e) => setAddForm((p) => ({ ...p, name: e.target.value }))} /></div><div><Label className="mb-2 block">{t("adminUsers.email", "Email")}</Label><Input type="email" value={addForm.email} onChange={(e) => setAddForm((p) => ({ ...p, email: e.target.value }))} /></div></div><div className="grid grid-cols-1 gap-3 md:grid-cols-2"><div><Label className="mb-2 block">{t("adminUsers.phone", "Phone")}</Label><Input value={addForm.phone} onChange={(e) => setAddForm((p) => ({ ...p, phone: e.target.value }))} /></div><div><Label className="mb-2 block">{t("adminUsers.role", "Role")}</Label><Select value={addForm.role} onValueChange={(v: UserRole) => setAddForm((p) => ({ ...p, role: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="user">{t("adminUsers.citizen", "Citizen")}</SelectItem><SelectItem value="officer">{t("adminUsers.officer", "Officer")}</SelectItem><SelectItem value="admin">{t("adminUsers.admin", "Admin")}</SelectItem></SelectContent></Select></div></div>
          {addForm.role === "officer" && <div><Label className="mb-2 block">{t("adminUsers.department", "Department")}</Label><Select value={addForm.departmentId} onValueChange={(v) => setAddForm((p) => ({ ...p, departmentId: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">{t("adminUsers.selectDepartment", "Select department")}</SelectItem>{departments.map((d) => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent></Select></div>}
          <div className="rounded-md border p-3"><div className="mb-2 flex items-center gap-2"><Checkbox checked={addForm.autoGeneratePassword} onCheckedChange={(c) => setAddForm((p) => ({ ...p, autoGeneratePassword: Boolean(c) }))} /><span className="text-sm">{t("adminUsers.autoGeneratePassword", "Auto-generate password")}</span></div>{!addForm.autoGeneratePassword && <Input type="password" placeholder={t("adminUsers.customPassword", "Custom password")} value={addForm.password} onChange={(e) => setAddForm((p) => ({ ...p, password: e.target.value }))} />}<div className="mt-2 flex items-center gap-2"><Checkbox checked={addForm.sendWelcome} onCheckedChange={(c) => setAddForm((p) => ({ ...p, sendWelcome: Boolean(c) }))} /><span className="text-sm">{t("adminUsers.sendWelcomeEmail", "Send welcome email")}</span></div></div>
        </div>
        <DialogFooter><Button variant="outline" onClick={() => setAddOpen(false)}>{t("common.cancel", "Cancel")}</Button><Button disabled={saving} onClick={async () => { if (!addForm.name || !addForm.email || !addForm.phone) return toast.error(t("adminUsers.error.nameEmailPhoneRequired", "Name, email, phone are required")); if (addForm.role === "officer" && addForm.departmentId === "none") return toast.error(t("adminUsers.error.departmentRequiredOfficers", "Department required for officers")); if (!addForm.autoGeneratePassword && addForm.password.length < 8) return toast.error(t("adminUsers.error.passwordMin8", "Password must be at least 8 chars")); setSaving(true); try { const res = await adminService.createUser({ name: addForm.name, email: addForm.email, phone: addForm.phone, role: addForm.role, departmentId: addForm.role === "officer" ? addForm.departmentId : undefined, autoGeneratePassword: addForm.autoGeneratePassword, password: addForm.autoGeneratePassword ? undefined : addForm.password, sendWelcome: addForm.sendWelcome }); const temp = res?.data?.tempPassword; toast.success(temp ? t("adminUsers.createdWithTemp", "User created. Temp password: {password}").replace("{password}", temp) : t("adminUsers.created", "User created")); setAddOpen(false); setAddForm({ name: "", email: "", phone: "", role: "user", departmentId: "none", autoGeneratePassword: true, password: "", sendWelcome: true }); await loadUsers(true); } catch (err: unknown) { toast.error(parseErr(err, t("adminUsers.error.createUser", "Failed to create user"))); } finally { setSaving(false); } }}>{t("adminUsers.createUser", "Create User")}</Button></DialogFooter>
      </DialogContent></Dialog>

      <Dialog open={!!confirm} onOpenChange={() => setConfirm(null)}><DialogContent><DialogHeader><DialogTitle className="flex items-center gap-2"><ShieldAlert className="h-5 w-5 text-amber-600" />{t("adminUsers.confirmAction", "Confirm Action")}</DialogTitle><DialogDescription>{confirm?.type === "ban" && t("adminUsers.confirm.ban", "Ban {name}?").replace("{name}", String(confirm.userName || ""))} {confirm?.type === "unban" && t("adminUsers.confirm.unban", "Unban {name}?").replace("{name}", String(confirm.userName || ""))} {confirm?.type === "reset" && t("adminUsers.confirm.reset", "Reset password for {name}?").replace("{name}", String(confirm.userName || ""))} {confirm?.type === "bulk" && t("adminUsers.confirm.bulk", "Apply {action} on {count} users?").replace("{action}", String(confirm.bulkAction || "")).replace("{count}", String(selectedIds.length))} {confirm?.type === "delete" && t("adminUsers.confirm.delete", "Delete {name}? This cannot be undone.").replace("{name}", String(confirm.userName || ""))}</DialogDescription></DialogHeader>{confirm?.type === "ban" && <div><Label className="mb-2 block">{t("adminUsers.reason", "Reason")}</Label><Textarea value={reason} onChange={(e) => setReason(e.target.value)} /></div>}<DialogFooter><Button variant="outline" onClick={() => setConfirm(null)}>{t("common.cancel", "Cancel")}</Button><Button variant={confirm?.type === "ban" || confirm?.type === "delete" ? "destructive" : "default"} onClick={confirmAction} disabled={saving}>{t("common.confirm", "Confirm")}</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
};
